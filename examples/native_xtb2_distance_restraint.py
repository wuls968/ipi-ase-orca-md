...
def build_config(
    *,
    ...
    job_name: str = "native_xtb2_distance_restraint",
    ...
    return replace(
        ...
        plumed=replace(
            ...
            source_string=(
                ...
                "restraint: RESTRAINT ARG=dOH AT=0.95 KAPPA=5.0\n"
                "PRINT STRIDE=1 ARG=dOH,restraint.bias FILE=COLVAR\n"
            ),
            bias_name="plumed",
            bias_nbeads=1,
            plumed_step=0,
            ...
            plumed_extras=("dOH", "restraint.bias"),
        ),
        advanced=replace(
            config.advanced,
            fix_com=True,
...
def build_arg_parser() -> ArgumentParser:
    ...
    parser.add_argument("--job-name", default="native_xtb2_distance_restraint")
    parser.add_argument(
        "--smoke",
        action="store_true",
        help=f"Use a fast {SMOKE_STEPS}-step PLUMED restraint smoke test.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.smoke and job_name == "native_xtb2_distance_restraint":
        job_name = "native_xtb2_distance_restraint_smoke"
